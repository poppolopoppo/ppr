...
- **Collector**: Generic push-back sink (`Collector<Args...>` derives from `std23::function_ref<std::error_code(...)>`) with `append(first, last)` and tuple support — lets producers fill any container through one callable type.
