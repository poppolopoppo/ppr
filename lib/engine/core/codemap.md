...
- **Logger**: `Core.Logger.cppm/.cpp` — `Log::ELevel` (`debug = 0` … `fatal`, emoji `toString` per char width), `Category` (`immediate`/`break_on_error`/`break_on_warning` flags), `Emitter::shouldBreakDebugger`, `Entry` (null-terminated `string_view` + `opaque::Block` params + `TimePoint`/`ThreadId`). `Handler` owns a ~2 MiB `RawChannel` (override via `PPR_LOG_BUFFER_SIZE`), mutex-guarded writer `Policy`, and a `std::jthread` drain loop; `log` placement-news the entry plus params slab and auto-flushes on `immediate`, `logRaw` embeds a null-terminated message copy, `defaultWriter_` prints elapsed/`toString` line plus source location on `warning`+ with conditional flush. Global atomic minimum verbosity (`setMinimumVerboseLevel`) and `isLogEmitterVisible_` gate (breakpoint → category → global) filter before enqueue; dtor closes the channel to stop the worker.
- **Timer**: `Core.Timer.cppm/.cpp` — `TimePoint`/`TimeSpan`/`TimeDuration` aliases, `time::now`/`since`/`seconds` overloads, `ITimerClock` (protected dtor, `const` `now()`, `steady()` singleton) plus testable `TimerExplicitClock` (`tick`/`reset`/`last`). `TimerManager` keeps an atomic last-tick plus mutex-guarded min-heap (`push_heap`/`pop_heap` with `std::greater`); `schedule` runs past-due callbacks inline, `tick` drains due events into a `StableVectorInplace` before invoking outside the lock; `mainTimer()` singleton backs `context::withDeadline/withTimeout`. `std::formatter<TimeSpan>` prints ns/µs/ms/s; `opaqueValue` hooks for `TimePoint`/`TimeSpan`.
