...
namespace pP::io {
    ...
    IoPort &IoPort::operator=(IoPort &&other) noexcept {
        if (this != &other) {
            if (m_handle != nullptr) {
                hal::io::deinit(m_handle);
            }
    ...
    std::expected<IoFile, std::error_code> IoPort::open(const std::filesystem::path &path, const hal::io::OpenFlags flags) noexcept {
        try {
