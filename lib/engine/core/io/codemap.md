...
- **DirectoryWatcher** (`Core.Io.FileWatcher.cppm/.cpp`, `final : IEvent`, move-deleted): `PulseEvent m_changed` + fixed inline buffers (256 `WatchEvent`s, 16 KiB names, 64 KiB raw). Constructor init list follows declaration order (`m_recursive` before `m_root`, `-Wreorder`-clean on Clang). `poll(ec)`/`wait(ec)` pull raw bytes via `hal::io::pollWatch`/`waitWatch`, normalize with `parseWatchEvents` into cached `FileChange{Action, filename}` records (`added`/`removed`/`modified`/`renamed_old`/`renamed_new`); `changes()`, `hadOverflow()`, `hadError()`, `isOpen()`, `root()` report health.
