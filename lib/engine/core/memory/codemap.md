...
- **safe_object / safe_ptr** (`namespace pP`, in `memory.pointer`): debug-mode lifetime checker — `safe_object` (protected throwing dtor; move/copy start the new instance unobserved and require the source to be unobserved) tracks referencers by `SparseKeyId` (`incSafeRef`/`decSafeRef` + `incSafeRefIFP`/`decSafeRefIFP` null-tolerant helpers, `SparseVector<Referencer>` registry with callstack under `PPR_ENABLE_SAFE_OBJECT_TRACKING`), `safe_ptr` asserts no copy outlives the object (`unique_ptr` ctor + CTAD, `upcast()` transfers the referencer key into the new pointer, full copy/move/converting ops); release mode is a zero-overhead raw pointer (compile-enforced pointer-sized, trivially copyable/destructible). Enabled via `PPR_ENABLE_SAFE_OBJECT_TRACKING` (SparseVector-backed registry).
