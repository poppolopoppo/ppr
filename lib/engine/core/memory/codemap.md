...
- **safe_object / safe_ptr** (`namespace pP`, in `memory.pointer`): `safe_object` (protected throwing dtor; move/copy start the new instance unobserved and require the source to be unobserved) tracks referencers by `SparseKeyId` (`incSafeRef`/`decSafeRef` + `incSafeRefIFP`/`decSafeRefIFP` null-tolerant helpers, `SparseVector<Referencer>` registry with callstack under `PPR_ENABLE_SAFE_OBJECT_TRACKING`). `safe_ptr` exposes the exact same API at `PPR_ENABLE_DEBUG=0` as at `=1` at zero overhead — implicit ctors (incl. `unique_ptr`-observing + CTAD), `T *const` reset/assign, const-tolerant `==` (both operand orders), `convertible_to`-only converting constraints, trivial `upcast()`/`swap`; pointer-size/triviality enforced by `static_assert`s. Do NOT rewrite call sites per-config — asymmetry is a contract bug. Debug tracks referencers and asserts no copy outlives the object; release is a raw pointer.
