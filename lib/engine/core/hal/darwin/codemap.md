...
- `Core.HAL.darwin.Strings.cpp` — manual UTF-8/ASCII transcoding + shared `string_view → char*` overload, no OS API calls
...
- `Core.HAL.darwin.Io.cpp` — stub: kqueue/not-yet-implemented, throws unsupported (contrast the Linux raw-syscall io_uring backend)