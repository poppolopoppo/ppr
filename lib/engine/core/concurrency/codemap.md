...
- **Context** (`Core.Concurrency.Context.cppm/.cpp`): `IContext : IEvent` adds `error()` + `value(user_key)`; `SharedContext = shared_ptr<IContext>`. `context::` factories: `background()`, `withCancel`/`withCancelClause` (+ `CancelFunc`/`CancelClauseFunc` weak handles with `isValid()` liveness check), `withoutCancel` (sever cancel, keep values), `withAfterFunc` (run on cancel/destroy), `withValue`/`withValues` (opaque data), `withDeadline(Cause)`/`withTimeout(Cause)` taking `TimePoint`/`TimeSpan` by value and defaulting to `TimerManager::mainTimer()`. `RawChannel::EError` remains a plain enum (TODO: migrate to `errc` + custom `error_code` category).
