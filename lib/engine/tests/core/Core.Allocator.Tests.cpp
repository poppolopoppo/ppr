...
namespace pP::tests::detail {
    namespace Allocator {
        ...
        PPR_UNIT_TEST (overlap_boundaries) {
            ...
            PPR_TEST_ASSERT(mem::overlap(storage.data(), storage.size(), storage.data()));
            PPR_TEST_ASSERT(mem::overlap(storage.data(), storage.size(), storage.data() + 15u));
            PPR_TEST_ASSERT(!mem::overlap(storage.data(), storage.size(), storage.data() + 16u));

            PPR_TEST_ASSERT(mem::overlap(storage.data(), storage.size(), storage.data(), 1u));
            PPR_TEST_ASSERT(mem::overlap(storage.data(), storage.size(), storage.data() + 8u, 4u));
        ...
        PPR_UNIT_TEST (gpa_alignment_paths) {
            ...
            PPR_TEST_ASSERT(overaligned.ptr != nullptr);
            PPR_TEST_ASSERT(std::bit_cast<std::uintptr_t>(overaligned.ptr) % 64u == 0u);
        ...
        PPR_UNIT_TEST (fallback_prefers_primary_then_secondary) {
            ...
            PPR_TEST_ASSERT(primary.ptr != nullptr);
        ...
        PPR_UNIT_TEST (allocator_wrapper_forwards_and_force_ref) {
            ...
            PPR_TEST_ASSERT(&const_wrapped.materialize() == &backend);
            PPR_TEST_ASSERT(&wrapped.materialize() == &backend);

            const auto block = wrapped.allocateRaw(16u, max_align_v);
            PPR_TEST_ASSERT(block.ptr != nullptr);
            PPR_TEST_ASSERT(wrapped.owns(block.ptr, 16u));

            auto forced = wrapped.forceRef();
            PPR_TEST_ASSERT(&forced.materialize() == &backend);

            const auto extra = forced.allocateRaw(8u, max_align_v);
            PPR_TEST_ASSERT(extra.ptr != nullptr);
            PPR_TEST_ASSERT(forced.resizeRaw(extra.ptr, 8u, 4u));
            ...
            PPR_TEST_ASSERT(tmp.ptr != nullptr);
            wrapped.restore(mark);
            PPR_TEST_ASSERT(!wrapped.owns(tmp.ptr, 8u));
        ...
        PPR_UNIT_TEST (pmr_erasure_and_equality) {
            ...
            PPR_TEST_ASSERT(stateful == erased);

            const auto shrinkable = stateful.allocateRaw(24u, max_align_v);
            PPR_TEST_ASSERT(shrinkable.ptr != nullptr);
            PPR_TEST_ASSERT(stateful.resizeRaw(shrinkable.ptr, 24u, 16u));
            ...
            PPR_TEST_ASSERT(overaligned.ptr != nullptr);
            PPR_TEST_ASSERT(!stateless.resizeRaw(overaligned.ptr, overaligned.count, overaligned.count * 2u));
        ...
        PPR_UNIT_TEST (allocator_traits_operations) {
            ...
            PPR_TEST_ASSERT(widget != nullptr);
            PPR_TEST_ASSERT(widget->x == 42);
            wrapped.destroy(widget);
            PPR_TEST_ASSERT(Widget::destroyed == 1u);

            PPR_TEST_ASSERT(!wrapped.template relocate<int>(nullptr, 0u, 0u).ptr);

            const auto fresh = wrapped.relocate<int>(nullptr, 0u, 3u);
            PPR_TEST_ASSERT(fresh.ptr != nullptr);
            fresh.ptr[0] = 1;
            fresh.ptr[1] = 2;
            fresh.ptr[2] = 3;

            const auto shrunk = wrapped.relocate<int>(fresh.ptr, 3u, 2u);
            PPR_TEST_ASSERT(shrunk.ptr == fresh.ptr);
            PPR_TEST_ASSERT(shrunk.count == 2u);
            PPR_TEST_ASSERT(shrunk.ptr[0] == 1);
            PPR_TEST_ASSERT(shrunk.ptr[1] == 2);

            auto grown = wrapped.relocate<int>(shrunk.ptr, 2u, 5u);
            PPR_TEST_ASSERT(grown.ptr != nullptr);
            PPR_TEST_ASSERT(grown.count == 5u);
            PPR_TEST_ASSERT(grown.ptr[0] == 1);
            PPR_TEST_ASSERT(grown.ptr[1] == 2);
        ...
        PPR_UNIT_TEST (allocation_stateful_resize_create_destroy) {
            ...
            PPR_TEST_ASSERT(!empty.isValid());
            PPR_TEST_ASSERT(!empty.resize(backend, 0u));
            PPR_TEST_ASSERT(empty.resize(backend, 4u));
            PPR_TEST_ASSERT(empty.isValid());
            PPR_TEST_ASSERT(empty.count() == 4u);
            empty.deallocateAssumeNotEmpty(backend);
            PPR_TEST_ASSERT(!empty.isValid());

            mem::Allocation<int, RecordingAllocator> alloc(4u, backend);
            PPR_TEST_ASSERT(mem::Allocation<int, RecordingAllocator>::alignment() == std::align_val_t{alignof(int)});
            PPR_TEST_ASSERT(alloc.isValid());
            PPR_TEST_ASSERT(alloc.count() == 4u);
            PPR_TEST_ASSERT(alloc.size_bytes() == sizeof(int) * 4u);
            PPR_TEST_ASSERT(alloc.view().size() == 4u);
            PPR_TEST_ASSERT(alloc.owns(alloc.data(), sizeof(int) * 2u));

            for (std::size_t i = 0u; i < alloc.count(); ++i) {
                alloc.data()[i] = static_cast<int>(i);
            }

            PPR_TEST_ASSERT(!alloc.resize(backend, 8u));
            PPR_TEST_ASSERT(alloc.resize(backend, 2u));
            PPR_TEST_ASSERT(alloc.count() == 2u);
            PPR_TEST_ASSERT(alloc.data()[0] == 0);
            PPR_TEST_ASSERT(alloc.data()[1] == 1);
            ...
            PPR_TEST_ASSERT(created != nullptr);
            PPR_TEST_ASSERT(created->x == 99);
        ...
        PPR_UNIT_TEST (allocation_raii_and_relocate) {
            ...
            PPR_TEST_ASSERT(alloc.view().size() == 6u);

            mem::Allocation moved{std::move(alloc)};
            PPR_TEST_ASSERT(!alloc.isValid());
            PPR_TEST_ASSERT(moved.isValid());
            PPR_TEST_ASSERT(moved.data()[5] == 5);

            mem::Allocation<int, mem::GPA> assigned{};
            assigned = std::move(moved);
            PPR_TEST_ASSERT(!moved.isValid());
            PPR_TEST_ASSERT(assigned.isValid());
            PPR_TEST_ASSERT(assigned.data()[5] == 5);

            const auto relocated = assigned.relocate(12u);
            PPR_TEST_ASSERT(relocated.ptr == assigned.data());
            PPR_TEST_ASSERT(assigned.count() == 12u);
            PPR_TEST_ASSERT(assigned.data()[5] == 5u);
        ...
        PPR_UNIT_TEST (allocation_index_operator) {
            ...
            PPR_TEST_ASSERT(alloc[0] == 10);
            PPR_TEST_ASSERT(alloc[1] == 20);
            PPR_TEST_ASSERT(alloc[2] == 30);
            PPR_TEST_ASSERT(alloc[3] == 40);

            const auto &const_alloc = alloc;
            PPR_TEST_ASSERT(const_alloc[0] == 10);
...
namespace pP::tests {
    extern const UnitTest allocator = UnitTest::Named("allocator") / [](UnitTest::IRun &_) -> void {
        _.recurse({
            ...
            detail::Allocator::in_situ_one_shot_and_reuse,
            detail::Allocator::fallback_prefers_primary_then_secondary,
            detail::Allocator::threshold_routes_and_resizes_within_bucket,
            detail::Allocator::allocator_wrapper_forwards_and_force_ref,
            detail::Allocator::pmr_erasure_and_equality,
            detail::Allocator::allocator_traits_operations,
            detail::Allocator::allocation_stateful_resize_create_destroy,
            detail::Allocator::allocation_raii_and_relocate,
            detail::Allocator::allocation_create_destroy_non_trivial,
            detail::Allocator::allocation_index_operator,
