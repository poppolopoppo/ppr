module;
#include "pP/UnitTest.h"

module engine.tests.core;

import engine.core;
import std;

namespace pP::tests::detail {
    namespace String {
        namespace Helper {
            PPR_UNIT_TEST (char_helpers) {
                PPR_TEST_ASSERT(toLower('A') == 'a');
                PPR_TEST_ASSERT(toLower('z') == 'z');
                PPR_TEST_ASSERT(toLower('!') == '!');

                PPR_TEST_ASSERT(toUpper('a') == 'A');
                PPR_TEST_ASSERT(toUpper('Z') == 'Z');
                PPR_TEST_ASSERT(toUpper('!') == '!');

                PPR_TEST_ASSERT(isSpace(' '));
                PPR_TEST_ASSERT(isSpace('\n'));
                PPR_TEST_ASSERT(isSpace('\t'));
                PPR_TEST_ASSERT(!isSpace('a'));

                PPR_TEST_ASSERT(isDigit('0') && isDigit('9'));
                PPR_TEST_ASSERT(!isDigit('a'));

                PPR_TEST_ASSERT(isAlpha('a') && isAlpha('Z'));
                PPR_TEST_ASSERT(!isAlpha('1'));

                PPR_TEST_ASSERT(isAlnum('a') && isAlnum('5'));
                PPR_TEST_ASSERT(!isAlnum(' '));
            };

            PPR_UNIT_TEST (escape_functions) {
                constexpr auto n = charEscape('\n');
                PPR_TEST_ASSERT(n.view() == "\\n");

                constexpr auto quote = charEscape('"');
                PPR_TEST_ASSERT(quote.view() == "\\\"");

                constexpr auto normal = charEscape('x');
                PPR_TEST_ASSERT(normal.view() == "x");

                constexpr auto amp = xmlEscapeChar('&');
                PPR_TEST_ASSERT(amp.view() == "&amp;");

                constexpr auto lt = xmlEscapeChar('<');
                PPR_TEST_ASSERT(lt.view() == "&lt;");

                constexpr auto normal_xml = xmlEscapeChar('x');
                PPR_TEST_ASSERT(normal_xml.view() == "x");
            };

            PPR_UNIT_TEST (case_fold_char) {
                constexpr CaseFoldChar c('A');
                PPR_TEST_ASSERT(*c == 'a');
                PPR_TEST_ASSERT(c == 'a');
                PPR_TEST_ASSERT(c == CaseFoldChar('a'));
                PPR_TEST_ASSERT(c <=> 'b' == std::strong_ordering::less);
            };
        }

        PPR_UNIT_TEST (helpers){
            _.recurse({
                Helper::char_helpers,
                Helper::escape_functions,
                Helper::case_fold_char,
            });

        };

        namespace Literal {
            PPR_UNIT_TEST (static_interop) {
                constexpr string_literal lit = "test";
                constexpr static_string stat = "test";

                PPR_TEST_ASSERT(lit == stat);
                PPR_TEST_ASSERT(lit.view() == stat.view());
                PPR_TEST_ASSERT(hashValue(lit) == hashValue(stat));

                constexpr auto sub = stat.subrange<1, 3>();
                PPR_TEST_ASSERT(sub.view() == "est");
            };

            PPR_UNIT_TEST (basic_view_and_equality) {
                PPR_TEST_ASSERT(details::relocatable<string_literal>::value);
                constexpr string_literal a("hello");
                constexpr string_literal b("hello");
                PPR_TEST_ASSERT(a == b);
                constexpr auto sv = a.view();
                PPR_TEST_ASSERT(sv == "hello");
            };

            PPR_UNIT_TEST (empty_string) {
                constexpr static_string<0> empty_static = "";
                PPR_TEST_ASSERT(empty_static.size() == 0);
                PPR_TEST_ASSERT(empty_static.view().empty());
            };
        }

        PPR_UNIT_TEST (literal){
            _.recurse({
                Literal::static_interop,
                Literal::basic_view_and_equality,
                Literal::empty_string,
            });

        };

        namespace Static {
            PPR_UNIT_TEST (basic_view_and_equality) {
                constexpr static_string a = "hello";
                constexpr static_string b = "hello";
                PPR_TEST_ASSERT(a == b);
                const auto sv = a.view();
                PPR_TEST_ASSERT(sv == "hello");
            };

            PPR_UNIT_TEST (copy_and_move) {
                constexpr static_string a = "hello";
                constexpr static_string b = a;
                PPR_TEST_ASSERT(a == b);
                constexpr static_string c = std::move(a);
                PPR_TEST_ASSERT(b == c);
            };

            PPR_UNIT_TEST (range) {
                constexpr static_string a = "hello";
                static_assert(std::ranges::contiguous_range<decltype(a)>);
                constexpr std::size_t n = std::ranges::size(a);
                PPR_TEST_ASSERT(n == 5);
            };

            PPR_UNIT_TEST (concatenation) {
                constexpr static_string a = "hello";
                constexpr static_string b = " world!";
                constexpr static static_string m = a + b;
                const auto sv = m.view();
                PPR_TEST_ASSERT(sv == "hello world!");
            };

            PPR_UNIT_TEST (edge_cases) {
                constexpr static_string a = "a";
                constexpr static_string b = "b";
                PPR_TEST_ASSERT(a <=> b == std::strong_ordering::less);
                PPR_TEST_ASSERT(b <=> a == std::strong_ordering::greater);

                constexpr static_string hello = "hello";
                constexpr static_string world = "world";
                constexpr auto combined = hello + world;
                PPR_TEST_ASSERT(combined.view() == "helloworld");
                static_assert(combined.size() == 10);
            };
        }

        PPR_UNIT_TEST (static_string){
            _.recurse({
                Static::basic_view_and_equality,
                Static::copy_and_move,
                Static::range,
                Static::concatenation,
                Static::edge_cases,
            });

        };

        namespace Range {
            PPR_UNIT_TEST (compare) {
                constexpr std::string_view a = "hello";
                constexpr std::string_view b = "HeLlO";
                PPR_TEST_ASSERT(a != b);
                PPR_TEST_ASSERT(caseFold(a) == caseFold(b));
                PPR_TEST_ASSERT((a <=> b) == ('h' <=> 'H'));
                PPR_TEST_ASSERT((caseFold(a) <=> caseFold(b)) == std::strong_ordering::equal);
            };

            PPR_UNIT_TEST (promote) {
                constexpr auto a = caseFold("_ToTo!");
                PPR_TEST_ASSERT(a.size() == 6u);
                PPR_TEST_ASSERT(a == std::string_view("_ToTo!"));
                PPR_TEST_ASSERT(a == std::string_view("_toto!"));
                PPR_TEST_ASSERT(a == "_ToTo!");
                PPR_TEST_ASSERT(a == "_tOtO!");
                PPR_TEST_ASSERT(a != "!ToTo_");
                PPR_TEST_ASSERT((a <=> "_toto!") == std::strong_ordering::equal);
                PPR_TEST_ASSERT((a <=> "_tOTo!") == std::strong_ordering::equal);
                PPR_TEST_ASSERT((a <=> "_ZOTo!") == std::strong_ordering::less);
                PPR_TEST_ASSERT((a <=> "_aoTo!") == std::strong_ordering::greater);
                PPR_TEST_ASSERT((a <=> "_zoTo!") == std::strong_ordering::less);
                PPR_TEST_ASSERT((a <=> "_AOTo!") == std::strong_ordering::greater);
            };

            PPR_UNIT_TEST (composition) {
                constexpr std::string_view a = "hello";
                constexpr std::string_view b = "HeLlO";
                PPR_TEST_ASSERT(a != b);
                PPR_TEST_ASSERT(toLower(a) == toLower(b));
                PPR_TEST_ASSERT((a <=> b) == ('h' <=> 'H'));
                PPR_TEST_ASSERT((toLower(a) <=> toLower(b)) == std::strong_ordering::equal);
            };

            PPR_UNIT_TEST (compositions_advanced) {
                constexpr std::string_view input = "  HeLlO   WoRlD!  ";

                auto squeezed = squeezeSpaces(trim(input));
                const std::string intermediate(squeezed);
                PPR_TEST_ASSERT(intermediate == "HeLlO WoRlD!");

                const std::string title_case(titleCase(intermediate));
                PPR_TEST_ASSERT(title_case == "Hello World!");

                auto trimmed = trim(caseFold("  HeLlO   "));
                PPR_TEST_ASSERT(trimmed == "hello");
            };

            PPR_UNIT_TEST (hashing) {
                constexpr std::string_view a = "hello";
                constexpr std::string_view b = "HeLlO";
                constexpr std::string_view c = " hello   ";

                const hash_t ha = hashValue(a);
                const hash_t hb = hashValue(b);
                const hash_t hc = hashValue(c);
                PPR_TEST_ASSERT(ha != hb);
                PPR_TEST_ASSERT(hb != hc);
                PPR_TEST_ASSERT(hc != ha);

                const hash_t hai = hashValue(caseFold(a));
                const hash_t hbi = hashValue(caseFold(b));
                const hash_t hci = hashValue(caseFold(c));
                PPR_TEST_ASSERT(hai == hbi);
                PPR_TEST_ASSERT(hbi != hci);
                PPR_TEST_ASSERT(hci != hai);
                PPR_TEST_ASSERT(ha != hai);
                PPR_TEST_ASSERT(hb != hbi);
                PPR_TEST_ASSERT(ha != hbi);

                const hash_t hat = hashValue(trim(a));
                const hash_t hbt = hashValue(trim(b));
                const hash_t hct = hashValue(trim(c));
                PPR_TEST_ASSERT(hat != hbt);
                PPR_TEST_ASSERT(hbt != hct);
                PPR_TEST_ASSERT(hct == hat);
                PPR_TEST_ASSERT(hat == hai);
                PPR_TEST_ASSERT(hct == hai);

                PPR_TEST_ASSERT(hashValue("long") != hashValue("longer"));
            };

            PPR_UNIT_TEST (to_string_conversion) {
                constexpr std::string_view sv = "Test123";
                std::string s1(toLower(sv));
                PPR_TEST_ASSERT(s1 == "test123");

                std::string s2(capitalize(sv));
                PPR_TEST_ASSERT(s2 == "Test123");

                std::string s3(trim("  hello  "));
                PPR_TEST_ASSERT(s3 == "hello");
            };
        }

        PPR_UNIT_TEST (range){
            _.recurse({
                Range::compare,
                Range::promote,
                Range::composition,
                Range::compositions_advanced,
                Range::hashing,
                Range::to_string_conversion,
            });

        };

        namespace Lazy {
            PPR_UNIT_TEST (trim_functions) {
                constexpr std::string_view s1 = "   hello   ";
                PPR_TEST_ASSERT(trim(s1) == "hello");

                constexpr std::string_view s2 = "\t\n\r hello world \f\v";
                PPR_TEST_ASSERT(trim(s2) == "hello world");

                constexpr std::string_view s3 = "no spaces";
                PPR_TEST_ASSERT(trim(s3) == "no spaces");

                constexpr std::string_view empty = "";
                PPR_TEST_ASSERT(trim(empty).empty());

                constexpr std::string_view only_spaces = "   \t\n  ";
                PPR_TEST_ASSERT(trim(only_spaces).empty());
            };

            PPR_UNIT_TEST (truncate_and_filter) {
                constexpr std::string_view s = "hello world";
                PPR_TEST_ASSERT(truncate(s, 5) == "hello");
                PPR_TEST_ASSERT(truncate(s, 100) == s);

                auto only_alpha = filterChars(s, &isAlpha<char>);
                PPR_TEST_ASSERT(only_alpha == "helloworld");
            };

            PPR_UNIT_TEST (squeeze_spaces) {
                constexpr std::string_view s1 = "hello   world";
                PPR_TEST_ASSERT(squeezeSpaces(s1) == "hello world");

                constexpr std::string_view s2 = "   multiple     \t\n   spaces   ";
                PPR_TEST_ASSERT(squeezeSpaces(s2) == " multiple spaces ");

                constexpr std::string_view s3 = "no extra spaces";
                PPR_TEST_ASSERT(squeezeSpaces(s3) == "no extra spaces");

                constexpr std::string_view s4 = " \t\n\r\f\v ";
                PPR_TEST_ASSERT(squeezeSpaces(s4) == " ");
            };

            PPR_UNIT_TEST (title_case) {
                constexpr std::string_view s1 = "hello world";
                PPR_TEST_ASSERT(titleCase(s1) == "Hello World");

                constexpr std::string_view s2 = "the quick brown fox";
                PPR_TEST_ASSERT(titleCase(s2) == "The Quick Brown Fox");

                constexpr std::string_view s3 = "  leading spaces";
                PPR_TEST_ASSERT(titleCase(s3) == "  Leading Spaces");

                constexpr std::string_view s4 = "mixed123CASE";
                PPR_TEST_ASSERT(titleCase(s4) == "Mixed123case");
            };

            PPR_UNIT_TEST (string_capitalize) {
                constexpr std::string_view s1 = "hello";
                PPR_TEST_ASSERT(capitalize(s1) == "Hello");

                constexpr std::string_view s2 = "hELLO";
                PPR_TEST_ASSERT(capitalize(s2) == "Hello");

                constexpr std::string_view s3 = "";
                PPR_TEST_ASSERT(capitalize(s3).empty());

                constexpr std::string_view s4 = "123abc";
                PPR_TEST_ASSERT(capitalize(s4) == "123abc");
            };

            PPR_UNIT_TEST (string_escape) {
                constexpr std::string_view s = R"(hello "world" \n)";
                auto escaped = stringEscape(s);
                const std::string result(escaped);
                PPR_TEST_ASSERT(result == R"(hello \"world\" \\n)");
            };

            PPR_UNIT_TEST (xml_escape) {
                constexpr std::string_view s = R"(<tag attr="value & 'more'>)";
                auto escaped = xmlEscape(s);
                const std::string result(escaped);
                PPR_TEST_ASSERT(result == "&lt;tag attr=&quot;value &amp; &apos;more&apos;&gt;");
            };

            PPR_UNIT_TEST (hex_encode) {
                constexpr std::string_view s = "\x01\xAB\xFF";
                auto hexed = hexEncode(s);
                const std::string result(hexed);
                PPR_TEST_ASSERT(result == "01abff");
            };
        }

        PPR_UNIT_TEST (lazy){
            _.recurse({
                Lazy::trim_functions,
                Lazy::truncate_and_filter,
                Lazy::squeeze_spaces,
                Lazy::title_case,
                Lazy::string_capitalize,
                Lazy::string_escape,
                Lazy::xml_escape,
                Lazy::hex_encode,
            });

        };
    }
} // namespace pP::tests::detail

namespace pP::tests {
    const UnitTest strings = UnitTest::Named("strings") / [](UnitTest::IRun &_) -> void {
        _.recurse({
            detail::String::helpers,
            detail::String::literal,
            detail::String::static_string,
            detail::String::range,
            detail::String::lazy,
        });
    };

    const UnitTest &stringsTests() noexcept {
        return strings;
    }
} // namespace pP::tests
