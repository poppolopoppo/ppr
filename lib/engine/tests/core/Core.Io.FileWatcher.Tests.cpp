...
namespace pP::tests::detail {
    namespace File_watcher {
        ...
        PPR_UNIT_TEST (create_file) {
            ...
            {
                std::ofstream ofs(td.path / "test.txt");
                ofs << "hello";
            }
        ...
        PPR_UNIT_TEST (delete_file) {
            TempDir td;
            {
                std::ofstream ofs(td.path / "todelete.txt");
                ofs << "x";
            }
            std::this_thread::sleep_for(std::chrono::milliseconds(50));
        ...
        PPR_UNIT_TEST (changes_cached_until_next_poll) {
            ...
            {
                std::ofstream ofs(td.path / "cached.txt");
                ofs << "x";
            }
            PPR_TEST_ASSERT(pollUntilChanges(w));
        ...
        PPR_UNIT_TEST (i_event_interface) {
            ...
            {
                std::ofstream ofs(td.path / "event_test.txt");
                ofs << "x";
            }
            PPR_TEST_ASSERT(pollUntilChanges(w));
        ...
        PPR_UNIT_TEST (multiple_files) {
            ...
            {
                std::ofstream ofs(td.path / "a.txt");
                ofs << "a";
            }
            {
                std::ofstream ofs(td.path / "b.txt");
                ofs << "b";
            }
            {
                std::ofstream ofs(td.path / "c.txt");
                ofs << "c";
            }
...
namespace pP::tests {
    const UnitTest file_watcher = UnitTest::Named("file_watcher") / [](UnitTest::IRun &_) -> void {
        ...
        _.recurse(detail::File_watcher::poll_idempotent_no_changes);
        _.recurse(detail::File_watcher::changes_cached_until_next_poll);
