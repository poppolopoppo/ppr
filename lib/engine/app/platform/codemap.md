...
- Linux bring-up (detail in `glfw/codemap.md`): native headers are gated per OS (`glfw3native.h` only on Windows/Apple), backend partitions import each other for complete types instead of forward-declaring (`:platform.glfw.player` → `:platform.glfw.input`, umbrella → all three backends + `:application`), gamepad indices cross the `int`-vs-`size_t` boundary via `checked_cast`, and the error category is `static const` (not `constexpr`)
