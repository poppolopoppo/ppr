...
- **GlfwInput** implements `IInputService` — singleton (`GlfwInput::get()`) holds hot per-frame device state (`KeyboardDevice`, `MouseDevice`, `std::array<GamepadDevice, 4>`), routes messages through player graph, dispatches to global/pushed listeners
