...
| `/commit` | Launches the `git-commit-planner` skill against the current working tree. The skill handles its own subagent routing (git state via CLion MCP / direct `bash`, grouping + dependency-ordering judgment via background `@oracle`, plan emission in the orchestrator). Produces an agent-executable staging plan (`.slim/commit-plan.json`) with per-hunk patches and cumulative staging instructions; the skill **never** edits the git index or runs `git commit` — the user retains the final `git commit` op as the trust boundary. |
| `/push` | Launches the `git-push-planner` skill against the current branch's unpushed commits. The skill handles its own subagent routing (git state via CLion MCP / direct `bash`, secret-pattern scan via background `@explorer`, squash-pattern detection via background `@oracle`, commit-message format validation inline). The skill **never** runs `git push`, rewrites history, or modifies the working tree — its output is a proposed squash/rebase plan and pre-push checklist only. |
